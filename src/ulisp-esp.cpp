...
enum type { ZERO=0, SYMBOL=2, NUMBER=4, STREAM=6, CHARACTER=8, FLOAT=10, CSTRING = 12, STRING=14, PAIR=16 };  // STRING and PAIR must be last
enum token { UNUSED, BRA, KET, QUO, DOT };
enum stream { SERIALSTREAM, I2CSTREAM, SPISTREAM, SDSTREAM, SPIFFSSTREAM, WIFISTREAM };
enum page_flags { DIRTY, SWAPPED };
...
// Forward references
object *getobject(obj_vptr vptr);
...
obj_vptr getvptr(object *obj) { 
  return (uint)obj - (uint)&Heap + BASE_OFFSET;
}
...
void vfree(obj_vptr vptr) {
  ...
  if (offset == 0) {
    ...
    newspc->next = spc->next;
    newspc->size = spc->size + size;
    spc->next = newspc;
  }
}
...
object *getobject(obj_vptr vptr) {
  vptr -= BASE_OFFSET;
  //uint page = vptr / PAGE_SIZE;
  //uint offset = vptr % PAGE_SIZE;
  object* obj = (object *)&Heap[vptr];
  return obj;
}

obj_vptr addobject(object *obj, size_t size=sizeof(object)) {
  obj_vptr vptr = valloc(size);
  object *newobj = getobject(vptr);
  if (obj != newobj) memcpy(newobj, obj, size);
}
...
void gc (object *form, object *env) {
    ...
    free_space *next = FreeSpace->next;
    free(FreeSpace);
    FreeSpace = next;
  }
  initheap();
...
char *symbolname (symbol_t x) {
  ...
  else if (x >= BASE_OFFSET) return lookupsymbol(x);
  char *buffer = SymbolTop;
  buffer[3] = '\0';
  for (int n=2; n>=0; n--) {
...
int longsymbol (char *buffer) {
  ...
  return i + BASE_OFFSET; // First number unused by radix40
}
